...
const VipGateModal = ({ onClose }) => {
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-sm mx-4 rounded-2xl p-6 pb-8"
...
// ── Support Link (gated) ──────────────────────────────────────────────────────
const SupportLink = ({ href, collapsed, isVip0, onGate, hoverBg, hoverColor, iconColor, children, icon: Icon }) => (
  <a
    href={isVip0 ? undefined : href}
    target={isVip0 ? undefined : '_blank'}
    rel="noopener noreferrer"
    title={collapsed ? (isVip0 ? 'VIP members only' : children) : undefined}
    onClick={isVip0 ? (e) => { e.preventDefault(); onGate() } : undefined}
    className="flex items-center rounded-xl text-sm font-semibold transition-colors duration-150"
    style={{
      gap:            collapsed ? 0 : '0.75rem',
      padding:        collapsed ? '0.625rem' : '0.625rem 0.75rem',
    ...
    onMouseEnter={e => {
      e.currentTarget.style.background = isVip0 ? '#F5F3EF' : hoverBg
      e.currentTarget.style.color      = isVip0 ? '#9B9690' : hoverColor
    }}
    onMouseLeave={e => {
      e.currentTarget.style.background = 'transparent'
      e.currentTarget.style.color      = '#5C5652'
    }}
  >
    {isVip0
      ? <Lock size={17} style={{ flexShrink: 0, color: '#B5AFA9' }} />
      : <Icon style={{ flexShrink: 0, color: iconColor }} />
    }
    {!collapsed && (
      ...
      >
        ...
        >
          <button
            onClick={() => navigate('/')}
            className="flex items-center gap-3 min-w-0 cursor-pointer transition-opacity hover:opacity-80"
            style={{ overflow: 'hidden', flex: collapsed ? '0 0 auto' : 1, background: 'none', border: 'none', outline: 'none' }}
            aria-label="Go to Home"
          >
            <img
              src="/logo.jpeg"
        ...
        >
          ...
          {mainLinks.map(l => <NavItem key={l.to + l.label} {...l} collapsed={collapsed} />)}

          {!collapsed && (
            <p className="px-3 mt-5 mb-2 text-[10px] font-bold uppercase tracking-widest" style={{ color: '#B5AFA9' }}>Actions</p>
          )}
          {collapsed && <div className="my-3 mx-auto h-px w-8" style={{ background: '#ECEAE6' }} />}
          {actionLinks.map(l => <NavItem key={l.to + l.label} {...l} collapsed={collapsed} />)}
