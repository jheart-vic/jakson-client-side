...
import {
  ...
  ArrowDownLeft, ArrowUpRight, Wallet, KeyRound,
} from 'lucide-react'
...
import {
  ...
  adminGetUserSecurity, adminVerifySecurityAnswer, adminResetUserPassword,
} from '../../api/admin'
...
// Reset password modal has 3 steps:
// 'method'  — choose: verify answer OR force reset
// 'verify'  — enter security answer → get resetToken
// 'reset'   — enter new password (+ reason if force)
const RESET_STEP = { METHOD: 'method', VERIFY: 'verify', RESET: 'reset' }
...
const AdminUserDetail = () => {
  ...
  const [modal,         setModal]         = useState(null)   // 'credit'|'deduct'|'suspend'|'role'|'resetpw'
  ...
  const [resetStep,      setResetStep]      = useState(RESET_STEP.METHOD)
  const [securityInfo,   setSecurityInfo]   = useState(null)   // { question, hasSecurityAnswer }
  const [securityAnswer, setSecurityAnswer] = useState('')
  const [resetToken,     setResetToken]     = useState(null)   // returned after verify
  const [resetForm,      setResetForm]      = useState({ newPassword: '', confirmPassword: '', reason: '' })
  ...
  const openResetPassword = async () => {
    setResetStep(RESET_STEP.METHOD)
    setSecurityAnswer('')
    setResetToken(null)
    setResetForm({ newPassword: '', confirmPassword: '', reason: '' })
    setIsForceReset(false)
    setModal('resetpw')

    // Fetch the user's security question in the background
    try {
      const { data: d } = await adminGetUserSecurity(id)
      setSecurityInfo(d)
    } catch {
      setSecurityInfo({ question: null, hasSecurityAnswer: false })
    }
  }

  // ── Step 1 → verify security answer ──────────────────────
  const handleVerifyAnswer = async () => {
    if (!securityAnswer.trim()) return toast.error('Enter the customer\'s security answer')
    setWorking(true)
    try {
      const { data: d } = await adminVerifySecurityAnswer(id, securityAnswer.trim())
      setResetToken(d.resetToken)
      setResetStep(RESET_STEP.RESET)
      toast.success('Answer verified ✓')
    } catch (err) {
      toast.error(err.response?.data?.message || 'Answer did not match')
    } finally {
      setWorking(false)
    }
  }

  // ── Step 1 → skip to force reset ─────────────────────────
  const handleChooseForce = () => {
  ...
  const handleResetPassword = async () => {
    const { newPassword, confirmPassword, reason } = resetForm
    if (!newPassword || newPassword.length < 6)
      return toast.error('Password must be at least 6 characters')
    if (newPassword !== confirmPassword)
      return toast.error('Passwords do not match')
    if (isForceReset && (!reason || reason.trim().length < 5))
      return toast.error('Please provide a reason (required for force reset)')

    setWorking(true)
    try {
      const payload = isForceReset
        ? { newPassword, reason: reason.trim() }
        : { newPassword, resetToken, reason: reason.trim() }

      await adminResetUserPassword(id, payload)
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <button
          ...
          className="col-span-2 flex items-center justify-center gap-2 bg-orange-50 text-orange-600
                     border-2 border-orange-200 py-3 rounded-2xl text-sm font-bold
                     active:scale-95 transition-transform"
        >
      ...
      <Modal
        isOpen={modal === 'resetpw'}
        onClose={closeResetModal}
        title="🔐 Reset Password"
      >
        {/* STEP: METHOD SELECTION */}
        {resetStep === RESET_STEP.METHOD && (
          <div className="space-y-4">
            <p className="text-sm text-gray-500 text-center">
              How would you like to reset the password for{' '}
              <strong className="text-gray-800">{u.maskedPhone}</strong>?
            </p>

            {/* Security question info banner */}
            <div className={`rounded-2xl p-3 text-xs font-medium text-center
              ${securityInfo?.hasSecurityAnswer
                ? 'bg-primary-light text-primary'
                : 'bg-gray-100 text-gray-400'}`}>
              {securityInfo === null
                ? 'Loading security question…'
                : securityInfo.hasSecurityAnswer
                  ? `Security Q: "${securityInfo.question}"`
                  : 'No security question set for this account'}
            </div>

            <button
              onClick={() => setResetStep(RESET_STEP.VERIFY)}
              disabled={!securityInfo?.hasSecurityAnswer}
              className="w-full flex flex-col items-center gap-1 py-4 rounded-2xl border-2
                         border-primary/30 bg-primary-light text-primary text-sm font-bold
                         active:scale-95 transition-transform disabled:opacity-40 disabled:cursor-not-allowed"
            >
              <span>✅ Verify Security Answer</span>
              <span className="text-[11px] font-normal opacity-70">Customer remembers their answer</span>
            </button>

            <button
              onClick={handleChooseForce}
              className="w-full flex flex-col items-center gap-1 py-4 rounded-2xl border-2
                         border-orange-200 bg-orange-50 text-orange-600 text-sm font-bold
                         active:scale-95 transition-transform"
            >
              <span>⚡ Force Reset</span>
              <span className="text-[11px] font-normal opacity-70">Customer forgot everything — requires audit reason</span>
            </button>

            <button onClick={closeResetModal}
              className="w-full py-3 rounded-2xl border-2 border-gray-200 text-gray-500 text-sm font-bold">
              Cancel
            </button>
          </div>
        )}

        {/* STEP: VERIFY SECURITY ANSWER */}
        {resetStep === RESET_STEP.VERIFY && (
          <div className="space-y-4">
            <div className="bg-primary-light rounded-2xl p-3 text-center">
              <p className="text-xs font-bold text-primary">{securityInfo?.question}</p>
            </div>
            <div>
              ...
              <input
                type="text"
                placeholder="Enter the answer the customer gives you"
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setResetStep(RESET_STEP.METHOD)}
                disabled={working}
                className="flex-1 py-3 rounded-2xl border-2 border-gray-200 text-gray-500 text-sm font-bold disabled:opacity-50"
              >
                ← Back
        ...
        {/* STEP: NEW PASSWORD */}
        {resetStep === RESET_STEP.RESET && (
          <div className="space-y-4">
            <div className={`rounded-2xl p-3 text-center text-xs font-bold
              ${isForceReset ? 'bg-orange-50 text-orange-600' : 'bg-success-light text-success'}`}>
              {isForceReset ? '⚡ Force reset — audit reason required' : '✅ Identity verified via security answer'}
            </div>
            ...
            <div>
              ...
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wide block mb-1.5">
                  Reason * <span className="text-gray-400 font-normal">(logged for audit)</span>
                </label>
                <textarea
                  rows={2}
                  placeholder="e.g. Customer called support, verified identity via phone"
                  value={resetForm.reason}
                  onChange={e => setResetForm(f => ({ ...f, reason: e.target.value }))}
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-sm
                             text-gray-800 outline-none focus:border-primary transition-all resize-none"
                />
              </div>

            <div className="flex gap-3">
              <button
                onClick={() => setResetStep(isForceReset ? RESET_STEP.METHOD : RESET_STEP.VERIFY)}
                disabled={working}
                className="flex-1 py-3 rounded-2xl border-2 border-gray-200 text-gray-500 text-sm font-bold disabled:opacity-50"
              >
                ← Back
              ...
              <button
                ...
                disabled={working}
                className="flex-1 py-3 rounded-2xl bg-orange-500 text-white text-sm font-bold active:scale-95 disabled:opacity-50"
              >
                {working
                  ? <span className="inline-block w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin-slow" />
                  : 'Reset Password'}
