          ...
          >
            ...
            <div className="relative p-5 h-full flex flex-col justify-between">
              ...
              <button
                ...
                className="self-start bg-white/20 backdrop-blur-sm border border-white/30
...
const Dashboard = () => {
  ...
  useEffect(() => {
    // Wrap in async IIFE so setState calls inside loadBal are deferred
    ;(async () => { await loadBal() })()
  ...
  return (
    <div className="min-h-dvh bg-surface pb-24">
            ...
            {ACTIONS.map((a, i) => (
              ...
              >
                <div className="w-14 h-14 rounded-[18px] flex items-center justify-center"
                  style={{ backgroundColor: a.bg, boxShadow: `0 4px 12px ${a.color}22` }}>
                  <a.icon size={22} style={{ color: a.color }} strokeWidth={1.8} />
                </div>
                <span className="text-[11px] font-bold text-gray-500">{a.label}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="px-4 mt-3 animate-slide-up delay-200">
        <button className="w-full flex items-center gap-3 bg-white rounded-2xl p-4
                           shadow-card active:scale-[0.99] transition-transform border border-gray-100">
          ...
          <div className="flex items-center gap-3 p-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-white/75 text-xs mt-0.5 truncate">
                Code: <span className="font-bold">{user?.referralCode || '—'}</span>
              </p>
            </div>
            <button
              onClick={copyInvite}
              className="bg-white rounded-xl px-3 py-2 flex items-center gap-1.5
                         text-accent text-xs font-bold active:scale-95 transition-transform shrink-0"
            >
              <Copy size={11} /> Copy
            </button>
          </div>
      ...
      <div className="px-4 mt-4 mb-2 animate-slide-up delay-250">
        ...
        <div className="space-y-3">
          {NEWS.map(n => (
            <div key={n.id}
              className="bg-white rounded-2xl p-4 flex gap-3 shadow-card
                         active:scale-[0.99] transition-transform border border-gray-50">
        ...
        <div className="text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm mt-2 leading-relaxed">
              Join the official Telegram group to participate in events and receive exclusive{' '}
              <strong className="text-gray-700">bonus codes</strong>.
            </p>
          </div>
          ...
          <div className="bg-orange-50 border border-orange-100 rounded-2xl p-3 flex items-start gap-2 text-left">
            ...
            <p className="text-orange-600 text-xs font-medium">
              Disruptive behavior or spam will result in an immediate ban.
            </p>
          </div>
          <button onClick={() => setModal(null)} className="text-primary font-bold text-sm">
            OK, Got it
          </button>
        </div>
      </Modal>
      ...
      <Modal isOpen={modal === 'reward'} onClose={() => setModal(null)} title="Redeem Reward">
        <div className="space-y-4">
          ...
          <input
            ...
            className="w-full px-4 py-4 rounded-2xl text-center text-xl font-extrabold tracking-[0.3em]
                       border-2 border-primary bg-primary-light text-primary outline-none
                       placeholder:text-primary/30 placeholder:font-bold"
            autoCapitalize="characters"
          />
          <div className="flex gap-3">
            <button onClick={() => { setModal(null); setRewardCode('') }}
              className="flex-1 py-3.5 rounded-2xl border border-gray-200 bg-gray-50
                         text-gray-500 text-sm font-bold active:scale-95 transition-transform">
              Cancel
            </button>
        ...
        <div className="text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">
              Check in every day to earn <strong className="text-warning">$0.01</strong> and build your streak
            </p>
          </div>
