...
import {
  ShoppingCart, TrendingUp, Package, Zap, ChevronRight, Star
} from 'lucide-react'
...
const Invest = () => {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setMyInvests(invRes.data.investments.filter(i => i.status === 'in_progress'))
  ...
  const roi = (p)         => p.amount > 0 ? +((totalReturn(p) / p.amount) * 100).toFixed(1) : 0
  const ownedCount = (p)  => myInvests.filter(i => i.product?._id === p._id || i.product === p._id).length

  const displayProducts = activeTab === 'active'
    ? products.filter(p => myInvests.some(i => i.product?._id === p._id || i.product === p._id))
    : products
  ...
  return (
    <div className="min-h-dvh bg-surface pb-28">
      ...
      <button onClick={() => navigate('/main/invest-log')}
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-gray-700">Investment Records</span>
        </div>
      ...
      <div className="px-4 mt-4 space-y-4">
        {loading ? <Spinner /> : displayProducts.length === 0 ? (
          <div className="text-center py-16 text-gray-400">
            ...
            <p className="text-sm font-medium">No active investments yet</p>
            <button onClick={() => setActiveTab('all')} className="text-primary text-sm font-bold mt-2">
              Browse products →
            </button>
          </div>
          ...
          const tier = p.amount === 0 ? 'free'
            : p.amount <= 30  ? 'bronze'
            : p.amount <= 100 ? 'silver'
            : 'gold'

          const TIER_STYLE = {
            free:   { grad: 'from-emerald-500 to-teal-400',    badge: 'bg-emerald-100 text-emerald-700', label: '⭐ Free Trial' },
            bronze: { grad: 'from-primary to-blue-400',        badge: 'bg-blue-100 text-blue-700',       label: '🔵 Starter' },
            silver: { grad: 'from-violet-500 to-purple-400',   badge: 'bg-violet-100 text-violet-700',   label: '💜 Popular' },
            gold:   { grad: 'from-amber-500 to-orange-400',    badge: 'bg-amber-100 text-amber-700',     label: '🏆 Premium' },
          }[tier]
            ...
            <div key={p._id}
              ...
              <div className={`bg-linear-to-r ${TIER_STYLE.grad} px-4 py-3 flex items-center justify-between`}>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-white font-extrabold text-sm leading-tight">{p.name}</p>
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${TIER_STYLE.badge}`}>
                      {TIER_STYLE.label}
                    </span>
                  </div>
                </div>
                ...
                {/* Status pill */}
                {soldOut ? (
                  <span className="bg-white/20 text-white text-[10px] font-bold px-2 py-1 rounded-full">Sold Out</span>
                ) : p.isFree ? (
                  <span className="bg-white text-emerald-600 text-[10px] font-extrabold px-2.5 py-1 rounded-full">FREE</span>
                ) : (
              ...
              <div className="p-4">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  {[
                    { icon: '💰', label: 'Cost',        val: p.isFree ? 'Free'             : fmtUSD(p.amount),         bold: !p.isFree },
                    { icon: '📈', label: 'Daily Income', val: fmtUSD(p.dailyIncome),         bold: true  },
                    { icon: '📅', label: 'Cycle',        val: `${p.cycleDays} days`,         bold: false },
                    { icon: '🎯', label: 'Total Return', val: fmtUSD(totalReturn(p)),        bold: true  },
                  ].map(({ icon, label, val, bold }) => (
                    <div key={label} className="bg-gray-50 rounded-xl p-3">
                      <p className="text-[10px] text-gray-400 font-medium mb-1">{icon} {label}</p>
                      <p className={`text-sm ${bold ? 'font-extrabold text-primary' : 'font-bold text-gray-700'}`}>{val}</p>
                    </div>
                  ))}
                </div>
                ...
                {!p.isFree && (
                  <div className="mb-4">
                    <div className="flex justify-between text-[10px] text-gray-400 mb-1.5">
                      <span>ROI: <strong className="text-success">{roi(p)}%</strong></span>
                ...
                >
                  {soldOut
                    ...
                    : owned >= p.maxUnits
                    ? '✅ Max Units Reached'
                    : <><ShoppingCart size={15} /> Invest Now — {p.isFree ? 'Free' : fmtUSD(p.amount)}</>
                  }
                </button>
              </div>
            </div>
      ...
      {/* ── Confirm Modal ─────────────────────────── */}
      <Modal isOpen={!!selected} onClose={() => !buying && setSelected(null)} title="Confirm Investment">
        {selected && (
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 bg-primary-light rounded-2xl p-3">
              ...
              <div>
                ...
                <p className="text-xs text-primary font-medium mt-0.5">{selected.cycleDays}-day investment</p>
              </div>
            </div>
            ...
            <div className="space-y-2.5">
              {[
                { label: 'Investment cost',  val: selected.isFree ? 'Free' : fmtUSD(selected.amount), highlight: true },
                { label: 'Daily income',     val: fmtUSD(selected.dailyIncome) },
                { label: 'Cycle duration',   val: `${selected.cycleDays} days` },
                { label: 'Total return',     val: fmtUSD(totalReturn(selected)), highlight: true },
                { label: 'ROI',              val: `${roi(selected)}%` },
              ].map(({ label, val, highlight }) => (
                <div key={label} className="flex justify-between items-center py-2 border-b border-gray-50 last:border-0">
                  <span className="text-sm text-gray-400 font-medium">{label}</span>
                  <span className={`text-sm font-extrabold ${highlight ? 'text-primary' : 'text-gray-800'}`}>{val}</span>
                </div>
              ))}
            </div>
            ...
            <div className="flex gap-3">
              <button onClick={() => setSelected(null)} disabled={buying}
