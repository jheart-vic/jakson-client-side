...
// Reusable input wrapper — handles border highlight cleanly
const InputWrap = ({ children, state }) => {
  const borderClass =
    state === 'match'   ? 'border-success shadow-[0_0_0_3px_rgba(16,185,129,0.12)]' :
    state === 'error'   ? 'border-danger shadow-[0_0_0_3px_rgba(239,68,68,0.12)]'   :
    state === 'focused' ? 'border-primary shadow-[0_0_0_3px_rgba(26,159,212,0.15)]' :
                          'border-gray-200'
  return (
    <div className={`flex items-center gap-2.5 px-4 h-13 bg-white border-[1.5px] rounded-2xl transition-all duration-200 ${borderClass}`}>
      {children}
    </div>
  )
}
...
const Register = () => {
  ...
  const confirmState = !form.confirmPassword ? (focus === 'confirm' ? 'focused' : '') :
    form.password === form.confirmPassword ? 'match' : 'error'
        ...
        <div className="card-glass p-6 animate-scale-in">
          ...
          {step === 0 && (
            <form onSubmit={nextStep} className="space-y-3.5">
              ...
              {/* Phone */}
              <InputWrap state={focus === 'phone' ? 'focused' : ''}>
              ...
              {/* Password */}
              <InputWrap state={focus === 'password' ? 'focused' : ''}>
              ...
              <InputWrap state={confirmState}>
                ...
                <input
                  ...
                  className="flex-1 outline-none text-sm font-medium text-gray-800 placeholder:text-gray-400 bg-transparent min-w-0"
                />
                {/* Match indicator — fixed alignment */}
                {form.confirmPassword && (
                  <span className={`shrink-0 text-[11px] font-bold leading-none
                    ${form.password === form.confirmPassword ? 'text-success' : 'text-danger'}`}>
                    {form.password === form.confirmPassword ? '✓' : '✗'}
                  </span>
                )}
              </InputWrap>
              ...
              {/* Referral */}
              <div className={`flex items-center gap-2.5 px-4 h-13 bg-amber-50 border-[1.5px] rounded-2xl transition-all duration-200
                ${focus === 'referral' ? 'border-accent shadow-[0_0_0_3px_rgba(249,115,22,0.15)]' : 'border-amber-200'}`}>
              ...
              <button type="submit" className="btn btn-primary rounded-2xl h-13 w-full mt-1">
                Continue →
              </button>
            </form>
          )}
          ...
          {step === 1 && (
            <form onSubmit={nextStep} className="space-y-3.5">
              ...
              <div className="relative">
                <select
                  value={form.securityQuestionId} onChange={set('securityQuestionId')}
                  className="w-full h-13 px-4 pr-10 bg-white border-[1.5px] border-gray-200 rounded-2xl
                             text-sm font-medium text-gray-800 outline-none appearance-none
                             focus:border-primary focus:shadow-input transition-all"
                >
                  <option value="">Select a security question</option>
                  {questions.map(q => (
                    <option key={q.id} value={q.id}>{q.question}</option>
              ...
              <div className={`flex items-center gap-2.5 px-4 h-13 bg-white border-[1.5px] rounded-2xl transition-all duration-200
                ...
                <input
                  type="text" placeholder="Your answer" value={form.securityAnswer}
              ...
              <div className="bg-primary-light rounded-2xl p-3 text-xs text-primary font-medium leading-relaxed">
                💡 Remember this answer — it's the only way to reset your password if you forget it.
              </div>

              <button type="submit" className="btn btn-primary rounded-2xl h-13 w-full">
                Continue →
              </button>
            </form>
          )}
          ...
          {step === 2 && (
            <form onSubmit={handleSubmit} className="space-y-3.5">
              ...
              {/* Captcha input — normal size, centered digits */}
              <InputWrap state={focus === 'captcha' ? 'focused' : ''}>
                <input
                  type="text" placeholder="4-digit code" value={form.captchaAnswer}
                  onChange={set('captchaAnswer')} maxLength={4} inputMode="numeric"
                  onFocus={() => setFocus('captcha')} onBlur={() => setFocus('')}
                  className="flex-1 outline-none text-base font-bold tracking-[0.6em] text-gray-800
                             placeholder:text-gray-400 placeholder:tracking-normal bg-transparent text-center min-w-0"
                />
              </InputWrap>

              <button type="submit" disabled={loading} className="btn btn-primary rounded-2xl h-13 w-full">
                {loading
