...
const InputWrap = ({ children, state }) => {
    ...
    return (
        <div
            className={`flex items-center gap-2.5 px-4 h-13 bg-white border-[1.5px] rounded-2xl transition-all duration-200 ${borderClass}`}
        >
            {children}
        </div>
    )
...
const Register = () => {
    ...
    const [form, setForm] = useState({
        ...
        securityAnswer: '',
        captchaAnswer: '',
        referralCode: searchParams.get('c') || '',
    })
    ...
    const fetchCaptcha = useCallback(async () => {
        ...
        try {
            ...
            handleApiError(err, 'Failed to load captcha')
        } finally {
    ...
    useEffect(() => {
        getSecurityQuestions()
            ...
            .catch((err) => handleApiError(err, 'Failed to load security questions'))
    }, [])
        ...
        >
            ...
            <div className='flex-1 px-5 pb-10 overflow-y-auto'>
                <div className='card-glass p-6 animate-scale-in'>
                    ...
                    {step === 0 && (
                        <form onSubmit={nextStep} className='space-y-3.5'>
                            ...
                            {/* Phone */}
                            <InputWrap
                                state={focus === 'phone' ? 'focused' : ''}
                            >
                                <span className='text-xs font-bold text-gray-400 border-r border-gray-200 pr-3 shrink-0 leading-none'>
                                    +234
                                </span>
                                <Phone
                                    size={14}
                    ...
                    ${form.password === form.confirmPassword ? 'text-success' : 'text-danger'}`}
                            ...
                            {/* Referral */}
                            <div
                                className={`flex items-center gap-2.5 px-4 h-13 bg-amber-50 border-[1.5px] rounded-2xl transition-all duration-200
                ${focus === 'referral' ? 'border-accent shadow-[0_0_0_3px_rgba(249,115,22,0.15)]' : 'border-amber-200'}`}
                            ...
                            <button
                                type='submit'
                                className='btn btn-primary rounded-2xl h-13 w-full mt-1'
                            >
                                Continue →
                            </button>
                        </form>
                        ...
                        <form onSubmit={nextStep} className='space-y-3.5'>
                            ...
                            <div className='relative'>
                                <select
                                    ...
                                    onChange={set('securityQuestionId')}
                ...
                ${focus === 'answer' ? 'border-primary shadow-input' : 'border-gray-200'}`}
                            ...
                            <div className='bg-primary-light rounded-2xl p-3 text-xs text-primary font-medium leading-relaxed'>
                                💡 Remember this answer — it's the only way to
                    ...
                    {step === 2 && (
                        <form onSubmit={handleSubmit} className='space-y-3.5'>
                            ...
                            <div className='flex items-center gap-2.5'>
                                <div
                                    className='flex-1 h-12 bg-gray-50 rounded-xl border border-gray-200 overflow-hidden
