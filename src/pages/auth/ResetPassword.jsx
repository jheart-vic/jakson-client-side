...
const ResetPassword = () => {
    ...
    return (
      <div className="min-h-dvh flex flex-col items-center justify-center px-6"
        ...
        <div className="card-glass p-8 text-center space-y-4 w-full max-w-sm">
          ...
          <p className="text-gray-700 font-semibold">Invalid or expired reset link.</p>
          <Link to="/forgot-password">
            <button className="btn btn-primary rounded-2xl h-12 w-full">Start Over</button>
          </Link>
        </div>
      </div>
    )
  ...
  const confirmState = !form.confirm ? (focus === 'confirm' ? 'focused' : '') :
    form.password === form.confirm ? 'match' : 'error'
        ...
        <div className="card-glass p-6 animate-scale-in">
          {!done ? (
            ...
            <div className="text-center space-y-5 py-4">
              ...
              <button onClick={() => navigate('/login')} className="btn btn-primary rounded-2xl h-13 w-full">
                Sign In Now →
              </button>
            </div>
          )}
